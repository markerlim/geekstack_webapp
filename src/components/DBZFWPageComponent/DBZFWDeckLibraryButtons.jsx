...
const DBZFWDecklibrarybtn = () => {
  ...
  useEffect(() => {
    const fetchDecks = async () => {
      ...
      const querySnapshot = await getDocs(collection(db, `users/${uid}/dbzfwdecks`));
      const deckDocs = [];
  ...
  const handleOpenDeck = async (deck) => {
    navigate(`/dbzfwbuilder?deckUid=${deck.id}`);
  };

  const handleOpen = async (deckId) => {
    const cardsSnapshot = await getDocs(collection(db, `users/${currentUser.uid}/dbzfwdecks/${deckId}/dbzfwcards`));
    const cardsData = [];
  ...
  const handleDeleteDeck = async (deckId) => {
    try {
      await deleteDoc(doc(db, `users/${currentUser.uid}/dbzfwdecks`, deckId));
  ...
  return (
    ...
    >
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingLeft: 'calc(5vw)', paddingRight: 'calc(5vw)' }}>
        <span style={{ display: 'flex', alignItems: 'center' }}><Style style={{ marginRight: '10px' }} />DBZFW Decks</span>
...
export default DBZFWDecklibrarybtn;
