...
export const DBZFWLdrCardDrawer = ({ open, onClose, setSelectedImage, setSelectedCardid }) => {
    const [boosterFilter, setBoosterFilter] = useState("");
    const [colorFilter, setColorFilter] = useState("");
    const [slidePosition, setSlidePosition] = useState(100);
    const [dataList, setDataList] = useState([]); 
    const boosters = ['FB01','FB02','FS01','FS02','FS03','FS04'];
    const colors = ['Red', 'Blue', 'Green', 'Yellow'];

    const dbzldrurl = `https://ap-southeast-1.aws.data.mongodb-api.com/app/data-fwguo/endpoint/dragonballzLeader?secret=${process.env.REACT_APP_SECRET_KEY}`;
    ...
    const filteredDataList = dataList.filter(item => {
        ...
        const isNotLeaderAwaken = item.cardtype !== "LEADER";
        
        return matchesBooster && matchesColor && isNotLeaderAwaken;
    });
