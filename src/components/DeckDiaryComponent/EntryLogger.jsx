...
const EntryLogger = ({
  ...
  const [rounds, setRounds] = useState([
    { result: "", goFirst: "", cards: [null, null, null] },
  ]);
  const eventList =
    content === "unionarena"
      ? listofUATNY
  ...
  const handleLogSubmit = async () => {
    if (!eventDate || !eventName || !eventCaption || !country) {
      console.log(
        "Validation failed: Missing eventDate, eventName, or country."
      );
      return;
    }
    ...
    if (rounds.some((round) => !round.result || !round.goFirst)) {
      console.log(
        "Validation failed: Some rounds are missing a result or goFirst."
      );
      return;
    }
    ...
    try {
      await addDoc(
        collection(db, `users/${currentUser.uid}/${deckpath}/${deck.id}/logs`),
        {
          eventDate,
          eventName,
          eventCaption,
  ...
  const cardPickerDeckCover =
    (content === "onepiece" || content === "dragonballzfw") && deck?.deckcover
      ? deck.deckcover
      : null;
            ...
            {rounds.map((round, index) => (
              ...
              >
                ...
                <Select
                  value={round.goFirst}
                  onChange={(e) =>
                    updateRound(index, "goFirst", e.target.value)
                  }
                  displayEmpty
                  fullWidth
                  sx={{ width: "65px", height: "40px" }}
