  ...
  useEffect(() => {
    const fetchDeckData = async () => {
      try {
        ...
        const decks = [];
        for (let deckDoc of querySnapshot.docs) {
    ...
    const unsubscribe = onSnapshot(collection(db, "uniondecklist"), async (snapshot) => {
      const decks = [];
      for (let deckDoc of snapshot.docs) {
        const data = deckDoc.data();
        if (data.uid) {
          const userDoc = await getDoc(doc(db, 'users', data.uid));
          if (userDoc.exists()) {
          ...
          <Box sx={{ width: { xs: "80vw", sm: "500px" }, height: { sm: '200px' }, paddingTop: "20px", paddingBottom: "20px", paddingLeft: "20px", borderRadius: "20px", bgcolor: "#26252D", display: "flex", gap: "20px", flexDirection: "row", cursor: "pointer", position: "relative" }} onClick={() => {
            ...
            <Box sx={{ color: "#f2f3f8", display: "flex", flexDirection: "column", gap: "7px", justifyContent: "start", alignItems: "start" }}>
              ...
              <Box sx={{ fontSize: { xs: '16px', sm: "20px" }, display: "flex", alignItems: "center" }}><Box sx={{ width: { xs: "20px", sm: "30px" }, height: { xs: "20px", sm: "30px" }, overflow: 'hidden', display: 'flex', justifyContent: 'center', alignItems: 'center', borderRadius: { xs: "10px", sm: "15px" } }}>
                <Box component="img" sx={{ width: { xs: "20px", sm: "30px" } }} src={deck.photoURL} alt={deck.displayName} /></Box>{deck.displayName}</Box>
