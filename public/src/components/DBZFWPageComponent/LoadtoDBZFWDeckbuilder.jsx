...
const LoadtoDBZFWDeckbuilder = ({ handleDeckLoaded }) => {
  const { currentUser } = useAuth();
  const { setFilteredCards } = useDBZCardState();
  const [decks, setDecks] = useState([]);

  const handleDeckClick = async (deck) => {
    // Fetch the cards associated with the deck from the database
    const querySnapshot = await getDocs(collection(db, `users/${currentUser.uid}/dbzfwdecks/${deck.id}/dbzfwcards`));
...
export default LoadtoDBZFWDeckbuilder;
