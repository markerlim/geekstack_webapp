...
const UnionarenaDecks = ({decknamesearch}) => {
  ...
  const handleOpen = async (deck) => {
    const cardsSnapshot = await getDocs(
      collection(db, `users/${currentUser.uid}/decks/${deck.id}/cards`)
    );
    const cardsData = [];
    ...
    const logsSnapshot = await getDocs(
      collection(db, `users/${currentUser.uid}/decks/${deck.id}/logs`)
    );
    const logsData = [];
  ...
  const handleDeleteLog = async (logId) => {
    try {
      await deleteDoc(doc(db, `users/${currentUser.uid}/decks/${deck.id}/logs`, logId));
    ...
    >
      ...
      >
        <span style={{ display: "flex", alignItems: "center" }}>
          <Style style={{ marginRight: "10px" }} />UA Decks
        </span>
      </Box>
      {loading ? (
        <Box
...
export default UnionarenaDecks;
