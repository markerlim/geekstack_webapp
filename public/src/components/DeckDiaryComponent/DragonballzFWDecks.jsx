...
const DragonballzFWDecks = ({decknamesearch}) => {
  ...
  useEffect(() => {
    const fetchDecks = async () => {
      ...
      const querySnapshot = await getDocs(
        collection(db, `users/${uid}/dbzfwdecks`)
      );
      const deckDocs = [];
  ...
  const handleOpen = async (deck) => {
    const cardsSnapshot = await getDocs(
      collection(db, `users/${currentUser.uid}/dbzfwdecks/${deck.id}/dbzfwcards`)
    );
    const cardsData = [];
    ...
    const logsSnapshot = await getDocs(
      collection(db, `users/${currentUser.uid}/dbzfwdecks/${deck.id}/logs`)
    );
    const logsData = [];
  ...
  const handleDeleteLog = async (logId) => {
    try {
      await deleteDoc(doc(db, `users/${currentUser.uid}/dbzfwdecks/${deck.id}/logs`, logId));
    ...
    >
      ...
      >
        <span style={{ display: "flex", alignItems: "center" }}>
          <Style style={{ marginRight: "10px" }} />DBZFW Decks
        </span>
      </Box>
      {loading ? (
        <Box
        ...
        >
          ...
          {filteredDecks.length === 0 ? (
            ...
            >
              No Dragonballz FW Deck Created
            </Box>
          ) : (
...
export default DragonballzFWDecks;
