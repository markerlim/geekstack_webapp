...
export const CardDrawerNFV2 = ({ open, onClose, chosenCard, onSwipeLeft, onSwipeRight }) => {
  ...
  const shareImageToWhatsApp = () => {
    // Replace 'chosenCard.currentImage' with the URL of your webp image
    const imageUrl = chosenCard.urlimage;
        ...
        <Box flexGrow={1} display='flex' flexDirection='column' >
          <Box textAlign={"center"} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '5px' }}>
            ...
            <Modal open={openErrorLog} onClose={() => setOpenErrorLog(false)} sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
              <ErrorLog chosenCard={chosenCard} onSubmitSuccess={() => setOpenErrorLog(false)} />
            </Modal>
            <Box
              sx={{
            ...
            <Grid draggable="false">
              <Grid container rowSpacing={1} columnSpacing={1}>
                {showDetails && (<>
                  ...
                  <Grid item xs={9}>
                    <Box sx={{ backgroundColor: "#C8A2C8", color: "#000000", padding: 1 }}>{chosenCard.cardName}</Box>
                  </Grid>
                  <Grid item xs={3}>
                    <Box sx={{ backgroundColor: "#240056", color: "#f2f3f8", padding: 1 }}>ID:</Box>
                  </Grid>
                  <Grid item xs={9}>
                    <Box sx={{ backgroundColor: "#C8A2C8", color: "#000000", padding: 1 }}>{chosenCard.cardId}</Box>
                  </Grid>
                  <Grid item xs={3}>
                    <Box sx={{ backgroundColor: "#240056", color: "#f2f3f8", padding: 1 }}>Color:</Box>
                  </Grid>
                  <Grid item xs={9}>
                    <Box sx={{ backgroundColor: "#C8A2C8", color: "#000000", padding: 1 }}>{chosenCard.color}</Box>
                  </Grid>
                  <Grid item xs={3}>
                    <Box sx={{ backgroundColor: "#240056", color: "#f2f3f8", padding: 1 }}>Traits:</Box>
                  </Grid>
                  <Grid item xs={9}>
                    <Box sx={{ backgroundColor: "#C8A2C8", color: "#000000", padding: 1 }}>{chosenCard.traits}</Box>
                  </Grid>
                </>)}
                <Grid item xs={3}>
                ...
                {showDetails && (<>
                  ...
                  <Grid item xs={9}>
                    <Box sx={{ backgroundColor: "#C8A2C8", color: "#000000", padding: 1 }}>{chosenCard.trigger}</Box>
                  </Grid>
                </>)}
              </Grid>
            </Grid>
