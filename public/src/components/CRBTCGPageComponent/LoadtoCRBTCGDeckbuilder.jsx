...
const LoadtoCRBTCGDeckbuilder = ({ handleDeckLoaded }) => {
  const { currentUser } = useAuth();
  const { setFilteredCards } = useCRBCardState();
  const [decks, setDecks] = useState([]);

  const handleDeckClick = async (deck) => {
    // Fetch the cards associated with the deck from the database
    const querySnapshot = await getDocs(collection(db, `users/${currentUser.uid}/crbtcgdecks/${deck.id}/crbtcgcards`));
  ...
  const handleDeleteDeck = async (deckId) => {
    try {
      await deleteDoc(doc(db, `users/${currentUser.uid}/crbtcgdecks`, deckId));
  ...
  useEffect(() => {
    const fetchDecks = async () => {
      ...
      const querySnapshot = await getDocs(collection(db, `users/${uid}/crbtcgdecks`));
      const deckDocs = [];
...
export default LoadtoCRBTCGDeckbuilder;
