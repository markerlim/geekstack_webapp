...
const OPTCGExport = ({ filteredCards, selectedImage, currentUser }) => {
    ...
    return (
        <Box id="OPTCGExport" sx={{ width: '1840px', height: '1000px', padding: '40px', backgroundImage: 'url(images/articlebg/onepiecebgblack.jpg)', display: 'flex', flexDirection: 'row', alignItems: 'start', justifyContent: 'center', gap: '20px' }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', width: '1400px', justifyContent: 'center', alignItems: 'start' }}>
                {filteredCards.map((onepiece) => (
                    [...Array(onepiece.count || 1)].map((_, index) => (
                        <Box marginBottom="-4px" item key={onepiece.cardid + index}>
                            <img
                                loading="lazy"
                                src={onepiece.image}
                                draggable="false"
